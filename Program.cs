...
using (var scope = app.Services.CreateScope())
{
    ...
    try
    {
        ...
        try
        {
            ...
            // Check and add Direction column if it doesn't exist
            command.CommandText = @"
                SELECT COUNT(*)
                FROM INFORMATION_SCHEMA.COLUMNS
                WHERE TABLE_NAME = 'Goals' AND COLUMN_NAME = 'Direction'";

            columnCount = Convert.ToInt32(await command.ExecuteScalarAsync());

            if (columnCount == 0)
            {
                logger.LogInformation("Adding Direction column to Goals table...");

                // Add the column with default value 0 (Increase)
                command.CommandText = "ALTER TABLE Goals ADD Direction int NOT NULL DEFAULT 0";
                await command.ExecuteNonQueryAsync();

                // Set Direction for existing goals based on InitialValue and TargetValue
                // Direction = 1 (Decrease) if InitialValue > TargetValue, else 0 (Increase)
                command.CommandText = @"
                    UPDATE Goals
                    SET Direction = CASE
                        WHEN ISNULL(InitialValue, CurrentValue) > TargetValue THEN 1
                        ELSE 0
                    END";
                await command.ExecuteNonQueryAsync();

                logger.LogInformation("Direction column added successfully.");
            }
            else
            {
                logger.LogInformation("Direction column already exists.");
            }
        ...
        {
            logger.LogError(migrationEx, "Migration check failed, but continuing...");
